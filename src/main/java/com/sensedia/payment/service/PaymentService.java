...
public class PaymentService {
	...
	public void payment(PaymentRequest request) {
		...
		DebitEntity debit = debitRepository.findById(debitUUID).orElseThrow(() -> new UnprocessableEntityException(
				new MessageError(ErrorMessage.FIELD_VALUE_NOT_EXISTS, "Debit", request.getDebitId())));
		...
		for (int i = 0; i < installmentsToPay.size(); i++) {
			...
			if (isAntecipation) {
				paidValue = installment.getValue()
						.subtract(installment.getValue()
								.divide(new BigDecimal("100"), RoundingMode.HALF_UP)
								.multiply(BigDecimal.valueOf(debit.getDiscountPercentage())));
